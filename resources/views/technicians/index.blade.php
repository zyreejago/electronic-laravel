    ...
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <a href="{{ route('technicians.create') }}" class="btn btn-primary btn-lg shadow-sm">
                <i class="fas fa-user-plus me-2"></i>{{ __('Tambah Teknisi Baru') }}
            </a>
        </div>
    </x-slot>
                ...
                <div style="background: linear-gradient(90deg, #4e73df, #224abe);" class="text-white p-4">
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 class="card-title mb-0 fw-bold">
                            <i class="fas fa-users-cog me-2"></i>{{ __('Semua Teknisi') }}
                        </h5>
                    </div>
                </div>
            </div>
                    ...
                    <table class="table table-hover align-middle mb-0">
                        <thead class="bg-light">
                            <tr>
                                ...
                                <th class="border-0 text-uppercase small fw-bold p-3">{{ __('SPESIALISASI') }}</th>
                                    ...
                                    <td class="p-3">
                                        <div class="d-flex gap-2">
                                            ...
                                            <form action="{{ route('technicians.destroy', $technician) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-danger btn-sm shadow-sm" onclick="return confirm('{{ __('Apakah Anda yakin ingin menghapus teknisi ini?') }}')">
